...
import com.ynot.aaspaasdoctor.Webservice.GPSTracker;
...
public class EditConsultation extends AppCompatActivity implements OnMapReadyCallback, AdapterView.OnItemSelectedListener {
    ...
    String lat, longit, cityy, dist, pin, name, id, typee;
    CardView op;
    int status = 0;
    double latitude = 0;
    double longitude = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Edit Consultation");
        ...
        if (getIntent().hasExtra("type")) {
            ...
            type.setSelection(Integer.valueOf(typee));
        }
        ...
        if (getIntent().hasExtra("longi")) {
            longit = getIntent().getStringExtra("longi");
            longititude = longit;
        }
    ...
    private void saveConsultation() {
        StringRequest stringRequest = new StringRequest(Request.Method.POST, URLs.EDIT_CONSULTATION,
                    ...
                    public void onResponse(String response) {
                        ...
                        try {
                            ...
                            if (ob.getBoolean("status")) {
                                Toast.makeText(EditConsultation.this, ob.getString("message"), Toast.LENGTH_SHORT).show();
...
//        CameraUpdate cameraUpdate = CameraUpdateFactory.newLatLngZoom(new LatLng(Double.valueOf(getIntent().getStringExtra("lat")),Double.valueOf(getIntent().getStringExtra("longi")) ), 10);
...
//    }
    ...
    public void onMapReady(GoogleMap googleMap) {
        mMap=googleMap;
        LatLng latLng;
        latLng = new LatLng(Double.valueOf(getIntent().getStringExtra("lat")),Double.valueOf(getIntent().getStringExtra("longi")));
        mMap.addMarker(new MarkerOptions().position(latLng).title(getIntent().getStringExtra("name")));
        //mMap.moveCamera(CameraUpdateFactory.newLatLng(place.getLatLng()));
        mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(latLng, 16));
        latitiude = String.valueOf(latLng.latitude);
        longititude = String.valueOf(latLng.longitude);
